...
import random
import math
from src.train import optimize_QR_DQN
import wandb
import gc
...
parser = argparse.ArgumentParser(description="training atari with Hierarchical DQN")
...
parser.add_argument("--wandb_save_name", type = str, default = "HDQN-exp001")
...
env = gym.make('MontezumaRevenge-ram-v0').unwrapped
env.reset()
...
# Network loaded
# meta controller
meta_model = HDQN(screen_height, screen_width, n_actions, hidden_dims)
meta_target = HDQN(screen_height, screen_width, n_actions, hidden_dims)

policy_net = QR_DQN(screen_height, screen_width, n_actions, num_quant, hidden_dims)
target_net = QR_DQN(screen_height, screen_width, n_actions, num_quant, hidden_dims)
...
# update target network => weight equalize
...
if __name__ == "__main__":
    ...
    plt.savefig("./results/QR_DQN_loss_reward_curve.png")

    weights_save_dir = "./weights/qr_dqn_best_" + args['wandb_save_name'].split("-")[1] + ".pt"
